...
test('given: invalid values for clinic, when: clinic address is missing, then: appropriated error is returned', () => {
    const clinic = () => 
        new Clinic({doctors: undefined as unknown as Doctor[], address: "Geldenaaksebaan 335, 3001 Leuven", contactNumber: contactNumber, rating: rating})

    expect(clinic).toThrow('Doctor is required.');
});

test('given: invalid values for clinic, when: clinic address is missing, then: appropriated error is returned', () => {
    const clinic = () => 
        new Clinic({doctors: [], address: "", contactNumber: contactNumber, rating: rating})
...
test('given: invalid values for clinic, when: clinic contactNumber is missing, then: appropriated error is returned', () => {
    const clinic = () => 
        new Clinic({doctors: [], address: address, contactNumber: 0, rating: rating})
...
test('given: invalid values for clinic, when: clinic rating is missing, then: appropriated error is returned', () => {
    const clinic = () => 
        new Clinic({doctors: [], address: address, contactNumber: contactNumber, rating: 0})
